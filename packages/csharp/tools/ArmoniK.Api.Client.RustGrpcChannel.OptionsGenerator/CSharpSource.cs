...
namespace ArmoniK.Api.Client.RustGrpcChannel.OptionsGenerator
{
  ...
  internal static class CSharpSource
  {
    ...
    // The repository's own header, on every file it tracks. Emitted rather than left off because
    // the C# cleanup CI runs its profile with headers and would otherwise add one - and a file it
    // edits is not the one the schema renders, which the build then refuses.
...
";
                                ...
                                string schemaName)
    {
      ...
      // the file's own nature - while a licence header below it is still one, and is what the C#
      // cleanup CI would otherwise insert into a file that has to stay byte for byte rendered.
                               ...
                               string text)
    {
      ...
      {
        // Nothing after the `///` on an empty line: .editorconfig trims trailing whitespace, and
        // the C# cleanup CI would rewrite a file that has to stay byte for byte what is rendered.
        source.Append(indent)
