...
#[cfg(feature = "serde")]
const BOOLEAN_SPELLINGS: &str =
    "e.g. `true`, `True`, `1`, `yes`, `on`, or `false`, `0`, `no`, `off`";

/// The boolean `value` spells, `None` for a spelling this vocabulary does not cover.
///
/// Trimmed and case-folded, because the same option reaches this crate two ways - from an
/// environment variable, and from a document a .NET binder wrote, where `bool.ToString()` spells
/// it `True` - and a vocabulary that accepted one spelling and not the other would make the two
/// sources disagree about the same value.
///
/// Outside the `serde` feature, unlike its readers: the spellings are what an option means, not
/// how it is deserialized, and [`crate::utils::read_env_bool`] reads them without serde.
///
/// The list is wide because this library is read from more than one language, and each spells a
/// boolean its own way. `true` is Java's and JSON's, `True` C#'s and Python's, `1` what C++
/// prints without `boolalpha` and what a shell carries, `on` what an INI file, systemd and YAML
/// 1.1 use, and `y`, `t` and their opposites what YAML 1.1 and a good many command-line parsers
/// accept. Refusing any of them would make an option unusable from a binding that had no reason
/// to know which one this crate preferred.
pub(crate) fn boolean(value: &str) -> Option<bool> {
    match value.trim().to_ascii_lowercase().as_str() {
        "" | "0" | "f" | "n" | "no" | "off" | "false" | "disable" | "disallow" | "forbid" => {
            Some(false)
        }
        "1" | "t" | "y" | "on" | "yes" | "true" | "enable" | "allow" | "authorize" => Some(true),
        _ => None,
    }
}
