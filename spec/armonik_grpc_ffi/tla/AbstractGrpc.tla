...
(* FAIRNESS AND SPEC                                                       *)
(*                                                                         *)
(* Three kinds of conjuncts live here.                                     *)
(*                                                                         *)
(* RuntimeRelease and ChannelFinishClosing are promises of the library     *)
(* alone: the FFI refinement discharges them with its own threads, and     *)
(* nothing outside the process can withhold them.                          *)
(*                                                                         *)
(* NetworkSend and ReceiveStatus are promises the library shares with the  *)
(* peer, and the refinement cannot discharge either on its own:            *)
(* ReceiveStatus says a terminal arrives, which waits on the peer          *)
(* speaking, and NetworkSend says a submitted message leaves, which waits  *)
(* on the peer opening its flow-control window.  Fairness on the network   *)
(* is the hypothesis; what the library owes is the rest of the path -      *)
(* once a status is decided it reaches the ABI boundary, and once a window *)
(* opens the message goes.                                                 *)
(*                                                                         *)
(* DeliverInitialMetadata looks like a third such case and is not, and the *)
(* reason is stronger than the library being able to answer it: the only   *)
(* two actions that can retract its guard - CallCancel and                 *)
(* ChannelFinishClosing - write the head themselves, in the same step.  So *)
(* the guard holds until the head is delivered, and WF1 closes on one      *)
(* conjunct.  MetadataFairnessRequirement is where that contract is        *)
(* published: it lists WF(DeliverInitialMetadata) and nothing else, where  *)
(* the terminal's requirement lists four and SubmitProgress's names        *)
(* NetworkSend.                                                            *)
(*                                                                         *)
(* So what the implementation owes here is not that the head arrives but   *)
(* that a head is always producible: no level of the refinement has a      *)
(* variable for the head reaching the wire, and the obligation is that no  *)
(* active call with no delivered events is ever left with no head to hand  *)
(* over.  Reading the promise as a disjunction over the wire and the end of*)
(* the call invites the objection that the second branch waits on the peer;*)
(* the proof takes neither branch.                                         *)
(*                                                                         *)
(* The Deliver* conjuncts are continuing obligations of the caller: level  *)
(* 0 has no demand variable, so delivery is guarded by availability alone, *)
(* and a caller that stops reading falsifies them.  The trigger actions    *)
(* (CallStart, SendMessage, RuntimeBeginShutdown) carry no fairness at     *)
(* all: the library never promises that a trigger occurs, only what        *)
(* follows one.                                                            *)
(***************************************************************************)
