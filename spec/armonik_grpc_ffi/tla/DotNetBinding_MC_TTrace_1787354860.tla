---- MODULE DotNetBinding_MC_TTrace_1787354860 ----
...
_inv ==
    ~(
        ...
        /\
        events_delivered = ((call1 :> <<"INITIAL_METADATA">>))
        /\
        ...
        submitted = ((call1 :> <<>>))
        /\
        delivery_callback_running = ((call1 :> TRUE))
        /\
        resources_released_callback_running = ((rt1 :> FALSE))
        /\
        call_dispose_state = ((call1 :> "draining"))
        /\
        shutdown_event_emitted = ((rt1 :> FALSE))
        /\
        buffer_length = ((<<call1, b1>> :> 0 @@ <<call1, b2>> :> 0))
        /\
        pending_continuations = ((call1 :> 0))
        /\
        payloads_consumed_by_host = ((call1 :> 1))
    )
----
...
\* to `JsonSerialize`. For example, a sub-sequence of _TETrace.
    \* ASSUME
    \*     LET J == INSTANCE Json
    \*         IN J!JsonSerialize("DotNetBinding_MC_TTrace_1787354860.json", _TETrace)
...
\*
\*trace == IODeserialize("DotNetBinding_MC_TTrace_1787354860.bin", TRUE)
\*
\*=============================================================================
\*
...
trace == 
    ...
    ([buffer_send |-> (call1 :> (b1 :> 0 @@ b2 :> 0)),buffer_state |-> (call1 :> (b1 :> "none" @@ b2 :> "none")),runtime_root_live |-> TRUE,channel_state |-> (ch1 :> "open"),delivered |-> (call1 :> <<>>),buffers_held_by_host |-> (call1 :> 0),runtime_dispose_state |-> "active",write_dones_emitted |-> (call1 :> 0),channel_runtime |-> (ch1 :> rt1),status_pending |-> (call1 :> FALSE),events_delivered |-> (call1 :> <<"INITIAL_METADATA">>),send_closed |-> (call1 :> FALSE),cancel_requested |-> (call1 :> TRUE),buffer_charge |-> (<<call1, b1>> :> 0 @@ <<call1, b2>> :> 0),consumer_phase |-> (call1 :> "drain"),retry_state |-> (call1 :> "idle"),handle_released |-> (call1 :> FALSE),runtime_state |-> (rt1 :> "RUNNING"),resources_released_emitted |-> (rt1 :> FALSE),received |-> (call1 :> <<>>),memory_used |-> 0,call_state |-> (call1 :> "started"),runtime_destroyed |-> (rt1 :> FALSE),sent |-> (call1 :> <<>>),second_event_owed |-> (rt1 :> FALSE),call_root_live |-> (call1 :> TRUE),last_lend_status |-> (call1 :> "NONE"),write_done_callback_running |-> (call1 :> FALSE),call_token_published |-> (call1 :> TRUE),call_channel |-> (call1 :> ch1),shutdown_callback_running |-> (rt1 :> FALSE),submitted |-> (call1 :> <<>>),delivery_callback_running |-> (call1 :> TRUE),resources_released_callback_running |-> (rt1 :> FALSE),call_dispose_state |-> (call1 :> "draining"),shutdown_event_emitted |-> (rt1 :> FALSE),buffer_length |-> (<<call1, b1>> :> 0 @@ <<call1, b2>> :> 0),pending_continuations |-> (call1 :> 0),payloads_consumed_by_host |-> (call1 :> 0)]),
    ([buffer_send |-> (call1 :> (b1 :> 0 @@ b2 :> 0)),buffer_state |-> (call1 :> (b1 :> "none" @@ b2 :> "none")),runtime_root_live |-> TRUE,channel_state |-> (ch1 :> "open"),delivered |-> (call1 :> <<>>),buffers_held_by_host |-> (call1 :> 0),runtime_dispose_state |-> "active",write_dones_emitted |-> (call1 :> 0),channel_runtime |-> (ch1 :> rt1),status_pending |-> (call1 :> FALSE),events_delivered |-> (call1 :> <<"INITIAL_METADATA">>),send_closed |-> (call1 :> FALSE),cancel_requested |-> (call1 :> TRUE),buffer_charge |-> (<<call1, b1>> :> 0 @@ <<call1, b2>> :> 0),consumer_phase |-> ,retry_state |-> (call1 :> "idle"),handle_released |-> (call1 :> FALSE),runtime_state |-> (rt1 :> "RUNNING"),resources_released_emitted |-> (rt1 :> FALSE),received |-> (call1 :> <<>>),memory_used |-> 0,call_state |-> (call1 :> "started"),runtime_destroyed |-> (rt1 :> FALSE),sent |-> (call1 :> <<>>),second_event_owed |-> (rt1 :> FALSE),call_root_live |-> (call1 :> TRUE),last_lend_status |-> (call1 :> "NONE"),write_done_callback_running |-> (call1 :> FALSE),call_token_published |-> (call1 :> TRUE),call_channel |-> (call1 :> ch1),shutdown_callback_running |-> (rt1 :> FALSE),submitted |-> (call1 :> <<>>),delivery_callback_running |-> (call1 :> TRUE),resources_released_callback_running |-> (rt1 :> FALSE),call_dispose_state |-> (call1 :> "draining"),shutdown_event_emitted |-> (rt1 :> FALSE),buffer_length |-> (<<call1, b1>> :> 0 @@ <<call1, b2>> :> 0),pending_continuations |-> (call1 :> 0),payloads_consumed_by_host |-> (call1 :> 1)])
    >>
----
...
---- CONFIG DotNetBinding_MC_TTrace_1787354860 ----
CONSTANTS
    Messages = { m1 }
    CallIds = { call1 }
    ChannelIds = { ch1 }
...
ALIAS
    _expression
=============================================================================
\* Generated on Sat Aug 22 01:27:50 CEST 2026