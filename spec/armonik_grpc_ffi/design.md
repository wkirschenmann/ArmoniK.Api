...
- **SingleRuntime**: at most one runtime with state âˆˆ {RUNNING, STOPPING,
...
- **StoppingClosesChannels**: runtime STOPPING â‡’ each of its channels is closing or closed
- **ReleasedNoChannels**: runtime RELEASED â‡’ each of its channels is closed

**Call â†” channel â†” runtime link:**
...
- **ReleasedNoCalls**: runtime RELEASED â‡’ every call of every channel is terminal. It
...
- monotone runtime transitions NOT_INIT â†’ RUNNING â†’ STOPPING â†’ RELEASED, with
  STOPPING â†’ FAILED_UNQUIESCED as the only branch: each action's guard names the state
...
- **EventualShutdown**: runtime STOPPING â‡’ â—‡ RELEASED
...
- **SubmitProgress**: for every position i â‰¤ Len(submitted), eventually
  `sent[i] = submitted[i]` or the call terminated
- **DeliveryProgress**: for every position i â‰¤ Len(received), eventually
...
- **WriteDonesNeverExceedSends / RunningWriteDoneWasEmitted**: the send-side
  no-double-free â€” never more acquittals than accepted sends
...
- **WriteDoneFreesASlot**: `EmitWriteDone(c) â‡’ HasFreeSendSlot(c)'`. A host woken by a
...
- **SendsEventuallyAcquitted**: per send â€” the k-th accepted send is acquitted by its
  in-order WRITE_DONE, whose callback returns
- **PayloadsEventuallyConsumed**: per payload â€” each payload handed over is
