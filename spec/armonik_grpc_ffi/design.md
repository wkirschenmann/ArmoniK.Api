...
| `ci/check_state_literals.py` | Green: 13 typed state variables, every string literal compared against one of them admissible. A retired value neither fails to parse nor fails to type - the comparison is simply always false, so a guard becomes dead and a model constraint prunes more than intended while every property still reports clean. A constraint reading `call_dispose_state = "disposed"` after that value became `settled` shrank two configurations that way. The binding comes from the typing conjuncts rather than a table, so a renamed state is caught wherever it is still spelled |
